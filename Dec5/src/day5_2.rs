...
fn main() {
    ...
    println!("{:?}", max);
    let mut field = Compressed::<usize>::zero((max.x+1, max.y+1));
    for line in lines {
        println!("{:?}", line);
        if line.p1.x == line.p2.x {
            ...
            let pl;
            let pr;
            if line.p1.x < line.p2.x { pl  = line.p1; pr = line.p2; } else {pl  = line.p2; pr = line.p1;}
            if pl.y < pr.y {
                for i in 0..=(pr.x-pl.x) {
                    let cp = (pl.x+i, pl.y+i);
                    println!("y up {:?}", cp);
                    field.set(cp, &field.get(cp)+1);
                }
            } else if pl.y > pr.y {
                for i in 0..=(pr.x-pl.x) {
                    let cp = (pl.x+i, pl.y-i);
